...
type ParamType = 'color' | 'letter' | 'tier' | 'type' | 'move' ;
const paramTypes: ParamType[] = ['color', 'letter', 'tier', 'type', 'move'];
const paramTypeDexesKeys: Dict<Dict<KeyedDict<ParamType, string[]>>> = {};

const searchTypes: (keyof IParametersWorkerData)[] = ['pokemon'];

class OddishsOutliers extends ScriptedGame {
...
const commands: GameCommandDefinitions<OddishsOutliers> = {
	grab: {
		command(target, room, user) {
			...
			if (this.options.freejoin) {
				...
				user.say("You outlayed " + answer + " and advanced to the next round!");
			}
...
export const game: IGameFile<OddishsOutliers> = {
	aliases: ['oddishs', 'outliers'],
	challengeSettings: {
		botchallenge: {
			enabled: true,
			options: ['speed'],
	...
	class: OddishsOutliers,
	commandDescriptions: [Config.commandCharacter + 'lay [Pokemon]'],
	commands,
	defaultOptions: ['freejoin', 'points'],
	description: "Players outlay Pokemon that fills in  the given parameters each round!",
	name: "Oddish's Outliers",
	mascot: "Oddish",
